...
from config import DLTrainerConfig, BertEmbeddingLayerConfig, StaticEmbeddingLayerConfig
from utils.create import create_instance, create_optimizer, create_lr_scheduler
from utils.raw_data import load_raw_data
from dataset.loader import build_data_loaders
from utils.training import cal_accuracy
from utils.config import get_instance_name

class DLTrainer(Trainer):
    """
    Base Trainer class that provides methods to:
    1. Train deep models such as LSTM, MemoryAugmented LSTM, TextCNN, TextRCNN, and so on.
    2. Compute metrics against an evaluation set and use these metrics for model selection.
    3. Test trained models, compute and publish metrics against a blind test set.
    """
    ...
    def prepare(self):
        """Prepare the model and data for model input."""
        ...
        if self.checkpoint and self.config.load_dictionary_from_ckpt:
            print("Loading dictionary and label2id from checkpoint...")
        ...
        if isinstance(emb_conf, StaticEmbeddingLayerConfig):
            ...
            dictionary_or_tokenizer = AutoTokenizer.from_pretrained(emb_conf.model_dir)

        self.model = create_instance(self.config.model)
        if self.use_cuda:
            self.model = self.model.cuda()
        self.train_loader, self.valid_loader, self.test_loader = build_data_loader(
            raw_data.pairs,
    ...
    def validate(self):
        ...
        if val_loss < self.best_loss:
            print(f"Find the best model with loss {val_loss:.4f}")
            self.best_loss = val_loss
            if self.config.score_method == "loss":
                find_best_model = True

        if val_acc > self.best_acc:
            print(f"Find the best model with accuracy {val_acc:.4f}")
        ...
        print(f"Validation Loss: {val_loss:.4f}\t"
              ...
              f"Best Validation Accuracy: {self.best_acc*100:.2f}%")
